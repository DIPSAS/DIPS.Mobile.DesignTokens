...
public static class Command
{
    ...
    public static async Task<CommandResult> CaptureAsync(string commandPath, string arguments, string workingDirectory = null, bool verbose = false)
    {
        if(verbose)
        {
            Console.WriteLine($"Executing command (CaptureAsync) {commandPath} {arguments} in working directory {workingDirectory}");
        }
    ...
    public static async Task ExecuteAsync(string commandPath, string arguments, string workingDirectory = null, int success = 0, bool verbose = false)
    {
        if(verbose)
        {
            Console.WriteLine($"Executing command {commandPath} {arguments} in working directory {workingDirectory}");
        }
